...
// Start output buffering and suppress display errors to avoid corrupting binary output
ob_start();
ini_set('display_errors', '0');
error_reporting(0);

// Set properties
$excelku->getProperties()->setCreator("adimt")
                         ->setLastModifiedBy("adimt");

// Mengambil data dari tabel
        $bulan = isset($_POST['bulan']) ? trim($_POST['bulan']) : (isset($_GET['bulan']) ? trim($_GET['bulan']) : '');
        $tahun = isset($_POST['tahun']) ? trim($_POST['tahun']) : (isset($_GET['tahun']) ? trim($_GET['tahun']) : '');
...
$excelku->getActiveSheet()->setCellValue('A2', "PEMERINTAH KECAMATAN CIPARAY");
$excelku->getActiveSheet()->mergeCells('A3:H3');
$excelku->getActiveSheet()->setCellValue('A3', "KANTOR KECAMATAN CIPARAY");
...
$excelku->getActiveSheet()->setCellValue('A5', "Jl. Bojong Parigi No. 111, Desa Ciparay, Kecamatan Ciparay, Kabupaten Bandung 40381 ");
...
// Clean any output buffers to avoid corrupting the file stream
if (ob_get_length()) { @ob_end_clean(); } else { @ob_clean(); }
flush();
